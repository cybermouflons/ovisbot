...
class Ctf(commands.Cog):
    ...
    async def addchallenge(self, ctx, *params):
        try:
            ...
            logger.error(e)
            if ctf.challenges[-1].name == challenge_name:
                ctf.challenges.pop()
            await ctx.channel.send("Ουπς. Κάτι επήε λάθος.")
    ...
    async def solve(self, ctx):
        try:
            ...
            await ctx.channel.send(
                f'Άρκησες! This challenge has already been solved by {", ".join(challenge.solved_by)}!'
            )
        except Exception as e:
            logger.error(e)
            await ctx.channel.send("Εσαντανώθηκα. Δοκίμασε ξανά ρε παρέα μου.")
    ...
    async def workon(self, ctx, params):
        await ctx.channel.send(
            "`!ctf workon ` is not supported any more. Use `!ctf attempt` instead"
        )
    ...
    async def attempt(self, ctx, params):
        ...
        try:
            ...
            await ctx.channel.send(
                f"Ρε μπρο μου... Ελύσαμε το τουτο το challenge αφού."
            )
        except UserAlreadyInChallengeChannelException:
            await ctx.channel.send(
                f'Ρε αρφούι μου... Είσαι ήδη μέσα στη λιστούα. [{", ".join(challenge.attempted_by)}]'
            )
        except Exception as e:
            logger.error(e)
            await ctx.channel.send(
                f"Εν τα κατάφερα να σε βάλω μέσα στη λιστούα... Σόρρυ."
            )
